...
async def simulate_treatment_outcome_with_image(
    ...
    """
    print(f"[TREATMENT SIM] Starting simulation for diagnosis={diagnosis}, treatment={treatment_type}, timeframe={timeframe}")
    try:
        # Read image
        image_bytes = await image.read()
        print(f"[TREATMENT SIM] Read {len(image_bytes)} bytes from uploaded image")
        ...
        print(f"[TREATMENT SIM] Calling predict_outcome_enhanced with projected_improvement={projected_improvement}")
        try:
            ...
            print(f"[TREATMENT SIM] predict_outcome_enhanced completed successfully")
            print(f"[TREATMENT SIM] Result keys: {enhanced_result.keys()}")
            print(f"[TREATMENT SIM] After image size: {len(enhanced_result.get('after_image', b''))} bytes")
            print(f"[TREATMENT SIM] Timeline images count: {len(enhanced_result.get('timeline_images', []))}")
        except Exception as pred_error:
            print(f"[TREATMENT SIM] ERROR in predict_outcome_enhanced: {str(pred_error)}")
            import traceback
            traceback.print_exc()
            raise pred_error
        ...
        print(f"[TREATMENT SIM] Saving files to {UPLOAD_DIR}")

        # Save before image
        with open(before_path, 'wb') as f:
            f.write(image_bytes)
        print(f"[TREATMENT SIM] Saved before image: {before_path}")

        # Save final after image
        after_image_bytes = enhanced_result.get('after_image')
        if not after_image_bytes:
            print(f"[TREATMENT SIM] ERROR: No after_image in enhanced_result!")
            raise ValueError("No after_image generated by predictor")
        with open(after_path, 'wb') as f:
            f.write(after_image_bytes)
        print(f"[TREATMENT SIM] Saved after image: {after_path} ({len(after_image_bytes)} bytes)")

        # Save progressive timeline images
        timeline_files = []
        timeline_images = enhanced_result.get('timeline_images', [])
        print(f"[TREATMENT SIM] Processing {len(timeline_images)} timeline images")
        for idx, timeline_img in enumerate(timeline_images):
            timeline_filename = f"treatment_timeline_{current_user.id}_{timestamp}_{idx}.jpg"
            timeline_path = UPLOAD_DIR / timeline_filename
            img_bytes = timeline_img.get('image_bytes')
            if img_bytes:
                ...
                print(f"[TREATMENT SIM] Saved timeline image {idx}: week {timeline_img['weeks']}, {len(img_bytes)} bytes")
            else:
                print(f"[TREATMENT SIM] WARNING: Timeline image {idx} has no image_bytes")
        print(f"[TREATMENT SIM] Created {len(timeline_files)} timeline file entries")
        ...
        print(f"[TREATMENT SIM] SUCCESS! Returning response:")
        print(f"[TREATMENT SIM]   afterImage: {response['afterImage']}")
        print(f"[TREATMENT SIM]   timeline entries: {len(response['timeline'])}")
        print(f"[TREATMENT SIM]   projectedImprovement: {response['projectedImprovement']}%")
